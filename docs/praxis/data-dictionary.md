...
None of the nine files contains a native multi-label fault flag. Each row's label is derived as five independent binary per-component flags — `fan_fail`, `lpc_fail`, `hpc_fail`, `hpt_fail`, `lpt_fail` — using two rules combined:

- **Rule 1 ("when"):** `fail = 1` if `RUL ≤ 30` cycles, else `0`.
- **Rule 2 ("what"):** where Rule 1 is `1`, set the flag(s) for whichever component(s) that source file is mapped to (Akindoju, 2025, Table 4.1): DS01/DS02 → HPT; DS03 → HPT+LPT; DS04 → Fan; DS05 → HPC; DS06 → HPC+LPC; DS07 → LPT; DS08a/DS08c → all five.

This two-rule construction matches Akindoju's Section 4.2.1 methodology directly, not an independently derived scheme — see `deliverables/relabel_decision_akindoju_alignment.docx` and `deliverables/rul_and_fault_classification_approach.docx` for the full decision record and worked reasoning. It replaces the earlier three-file, pooled-healthy 4-class design recorded in `deliverables/multiclass_fault_classification_plan.docx`, which is kept only as a superseded historical record.
